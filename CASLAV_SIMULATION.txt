...
% -----------------------------------------------------------------
% Verze 4.5: Optimalizované nastavení bez komplikovaných komentářů
% 40 příletů + 40 odletů, 3NM separace, buffer 60s, animace 150x
% ROZŠÍŘENO O STATISTICKÉ ANALÝZY
...
ANIMATE_SPEED = 200;              % Rychlost animace - ZMĚNĚNO z 10 na 150

% Parametry generování letounů - ZMĚNĚNO
FIXED_NUM_AIRCRAFT = 40;          % ZMĚNĚNO z 15 na 40 příletů
FIXED_NUM_DEPARTURES = 40;        % ZMĚNĚNO z 5 na 40 odletů
ROUTE_PROBABILITY = [0.5, 0.5];   % Pravděpodobnost výběru trasy [IAF SUKAV, IAF PIMEK]

% Parametry separace - ZMĚNĚNO
MIN_SEPARATION_NM_AIR = 3;        % ZMĚNĚNO z 5 na 3 NM
...
THEORETICAL_HOURLY_CAPACITY = 60;  % Teoretická hodinová kapacita - upravte podle potřeby
...
% NOVÉ RYCHLOSTI PŘÍLETŮ - ZMĚNĚNO
...
buffer_time = 60; % ZMĚNĚNO z 300 na 60
...
if DRAW_ANIMATION
    figure('Name', 'TMA Čáslav - Optimalizované nastavení', 'Position', [50, 50, 1300, 900]);
    hold on;
    ...
    title(['TMA Čáslav - Optimalizace (40+40 letů) - Separace ', num2str(MIN_SEPARATION_NM_AIR) ,' NM']); 
    grid on; axis equal;
    ...
    disp('Spouštím animaci (150x rychlost)...');
...
if ~isempty(workload_data_for_graph) && any(workload_data_for_graph)
    ...
    title('Analýza pracovní zátěže řídícího - TMA Čáslav'); 
    grid on;
...
save_filename = sprintf('TMA_results_%s.mat', datestr(now, 'yyyymmdd_HHMMSS'));
save(save_filename, 'results');
disp(['Výsledky uloženy do souboru: ', save_filename]);

disp(' ');
disp('Simulace TMA Čáslav úspěšně dokončena!');
