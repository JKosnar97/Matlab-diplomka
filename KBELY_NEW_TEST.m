...
% Verze 2.0: S pokročilým modelem zátěže a integrovaným plánováním z Čáslavi
% Zachována geometrie Kbely, aplikovány vylepšené algoritmy
...
ANIMATE_SPEED = 10;               % Rychlost animace (kolikrát rychleji než reálný čas)
...
comm_load_params.initial_contact.duration_total_s = 10;
comm_load_params.initial_contact.controller_workload_points = 18; % 20×0.9
comm_load_params.initial_contact.timing_offset_s = 0;

comm_load_params.faf_report.duration_total_s = 9;
comm_load_params.faf_report.controller_workload_points = 22; % 25×0.9
comm_load_params.faf_report.timing_offset_s = 0;

comm_load_params.landing_clearance.duration_total_s = 8;
comm_load_params.landing_clearance.controller_workload_points = 27; % 30×0.9
comm_load_params.landing_clearance.timing_offset_s = -60;

% Odletové komunikace
comm_load_params.takeoff_clearance.duration_total_s = 10;
comm_load_params.takeoff_clearance.controller_workload_points = 27; % 30×0.9
comm_load_params.takeoff_clearance.timing_offset_s = -15;

comm_load_params.departure_contact.duration_total_s = 10;
comm_load_params.departure_contact.controller_workload_points = 18; % 20×0.9
comm_load_params.departure_contact.timing_offset_s = 0;

comm_load_params.exit_clearance.duration_total_s = 6;
comm_load_params.exit_clearance.controller_workload_points = 9; % 10×0.9
comm_load_params.exit_clearance.timing_offset_s = 0;
...
% Rychlostní profily a výpočet časů (zachováno z Kbely)
knots_to_nm_per_sec = 1/3600;
speed_profile_1 = struct('ENTRY_PR522', 230, 'EKROT', 160, 'FAF', 140, 'RWY_ARR', 130);
speed_profile_2 = struct('ENTRY_ELPON', 230, 'EKROT', 160, 'FAF', 140, 'RWY_ARR', 130);
speed_profile_dep = struct('RWY_DEP', 0, 'VECTOR1', 150, 'EKROT', 210, 'EXIT', 210);
...
disp(' ');
disp('--- Generování a plánování letů s pokročilým modelem zátěže ---');
...
% Volání nového integrovaného plánovacího algoritmu
...
if ~isempty(scheduled_flights)
    for i = 1:length(scheduled_flights)
        ...
        if strcmp(flight.type, 'arrival')
            ...
            aircraft_comm_events_for_dyn_sim(end+1) = struct('time', flight.landing_time + comm_load_params.landing_clearance.timing_offset_s, 'points', comm_load_params.landing_clearance.controller_workload_points, 'flight_id', flight.id, 'duration', comm_load_params.landing_clearance.duration_total_s);
        else
...
if DRAW_ANIMATION
    figure('Name','Dynamická simulace letů v TMA Kbely - Vylepšený model','Position',[100,100,1200,800]); hold on;
    ...
    title(['TMA Kbely - Vylepšený model zátěže - Separace ',num2str(MIN_SEPARATION_NM_AIR),' NM']); 
    grid on; axis equal;
...
if ~isempty(workload_data_for_stats) && any(workload_data_for_stats)
    figure('Name', 'Analýza zátěže řídícího - TMA Kbely', 'Position', [150, 150, 1100, 600]);
    ...
    title('Analýza pracovní zátěže řídícího - TMA Kbely (vylepšený model)');
    grid on;
...
disp(' ');
disp('Simulace TMA Kbely s vylepšeným modelem úspěšně dokončena!');
    ...
    for req_idx = 1:length(sorted_requests)
        ...
        for delay_s = 0:TIME_STEP_PLANNING_S:MAX_PLANNING_DELAY_S
            ...
            if strcmp(request.type, 'arrival')
                ...
                temp_comm_events_for_this_flight(end+1) = struct('time', current_flight_candidate.landing_time + comm_params.landing_clearance.timing_offset_s, 'points', comm_params.landing_clearance.controller_workload_points, 'flight_id', current_flight_candidate.id, 'duration', comm_params.landing_clearance.duration_total_s);
