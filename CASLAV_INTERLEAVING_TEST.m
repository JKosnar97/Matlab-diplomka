...
% -----------------------------------------------------------------
% Verze 4.7: Střídavé (interleaving) plánování příletů a odletů
% Kombinace komplexního modelu workloadu (S1) se střídavým plánováním (S3)
% ROZŠÍŘENO O SPECIÁLNÍ SEPARAČNÍ PRAVIDLA
...
ANIMATE_SPEED = 200;              % Rychlost animace

% Parametry generování letounů
FIXED_NUM_AIRCRAFT = 40;          % Počet příletů
FIXED_NUM_DEPARTURES = 40;        % Počet odletů
ROUTE_PROBABILITY = [0.5, 0.5];   % Pravděpodobnost výběru trasy [IAF SUKAV, IAF PIMEK]

% Parametry separace
MIN_SEPARATION_NM_AIR = 3;        % Jednotná minimální separace ve vzduchu pro všechny kombinace letů

% Pravidla pro separaci na dráze (časové hodnoty v sekundách)
...
disp(['Rychlosti: IAF-IF (', num2str(speed_profile_arr.IAF), '-', num2str(speed_profile_arr.IF), '), IF-FAF (', num2str(speed_profile_arr.IF), '-', num2str(speed_profile_arr.FAF), '), FAF-THR (', num2str(speed_profile_arr.FAF), '-', num2str(speed_profile_arr.RWY_ARR), ')']);
...
disp(['Jednotná separace: ', num2str(MIN_SEPARATION_NM_AIR), ' NM pro všechny kombinace letů']);
...
% NOVÁ FUNKCE: Střídavé plánování s komplexním modelem workloadu
[scheduled_flights, rejected_flights_info, final_workload_per_interval, runway_operations_log] = ...
    plan_flights_with_interleaved_workload_control(orig_arrival_times, orig_departure_times, ...
...
if DRAW_ANIMATION
    figure('Name', 'TMA Čáslav - Střídavé plánování', 'Position', [50, 50, 1300, 900]);
    hold on;
    ...
    title(['TMA Čáslav - Střídavé plánování - Separace ', num2str(MIN_SEPARATION_NM_AIR) ,' NM']); 
    grid on; axis equal;
    ...
    disp('Spouštím animaci (200x rychlost)...');
...
fprintf('\n--- PARAMETRY SIMULACE ---\n');
fprintf('Jednotná vzdušná separace: %.0f NM\n', MIN_SEPARATION_NM_AIR);
...
if ~isempty(workload_data_for_graph) && any(workload_data_for_graph)
    figure('Name', 'Analýza zátěže řídícího - TMA Čáslav (Střídavé plánování)', 'Position', [150, 150, 1100, 600]);
    ...
    title('Analýza pracovní zátěže řídícího - TMA Čáslav (Střídavé plánování)'); 
    grid on;
...
if ~isempty(scheduled_flights)
    for i = 1:length(scheduled_flights)
        ...
        if strcmp(flight.type, 'arrival')
            color_plot = 'b';
            if flight.route_index == 2, color_plot = 'c'; end
            ...
            plot(flight.cf314_time, y_pos, 'go', 'MarkerSize', 5, 'MarkerFaceColor', 'g');
...
legend_elements(end+1) = plot(NaN, NaN, '-co', 'MarkerFaceColor','c', 'LineWidth',2); legend_texts{end+1} = 'Přílet (Trasa 2)';
...
legend_elements(end+1) = plot(NaN, NaN, 'go', 'MarkerFaceColor','g'); legend_texts{end+1} = 'CF314';
legend_elements(end+1) = plot(NaN, NaN, 'ms', 'MarkerFaceColor','m'); legend_texts{end+1} = 'Opuštění (ERUSO)';
...
save_filename = sprintf('TMA_results_interleaved_%s.mat', datestr(now, 'yyyymmdd_HHMMSS'));
save(save_filename, 'results');
disp(['Výsledky uloženy do souboru: ', save_filename]);

disp(' ');
disp('Simulace TMA Čáslav se střídavým plánováním úspěšně dokončena!');

% ----------------- POMOCNÉ FUNKCE -------------------

% --- NOVÁ FUNKCE PRO STŘÍDAVÉ PLÁNOVÁNÍ S KOMPLEXNÍM MODELEM ---
function [planned_flights, rejected_flights_log, workload_profile, runway_ops_log] = ...
    plan_flights_with_interleaved_workload_control(orig_arrivals_t, orig_departures_t, ...
    ...
    for k = 1:length(all_movements_requests)
        ...
        elseif strcmp(current_movement_request.type, 'departure')
            ...
            [orig_cf314_time, orig_eruso_time] = calculate_departure_times(orig_takeoff_time, dep_route);
            orig_exit_time = orig_eruso_time;
            ...
            for delay_s = 0:TIME_STEP_PLANNING_S:MAX_PLANNING_DELAY_S
                ...
                current_flight_candidate.cf314_time = orig_cf314_time + delay_s;
                current_flight_candidate.eruso_time = orig_eruso_time + delay_s;
...
% --- Kontrola vzdušné separace pro všechny kombinace letů ---
...
% --- Používáme standardní funkci kontroly separací ze S1 ---
% check_all_air_separations_v3 kontroluje jednotnou 3 NM separaci pro všechny kombinace letů

% --- Pomocná funkce pro výpočet pozice odletu ---
function [cf314_time, eruso_time] = calculate_departure_times(takeoff_time, departure_route)
    cf314_time = takeoff_time + departure_route.segment_time(1);
    eruso_time = cf314_time + departure_route.segment_time(2);
end

% --- Hlavní funkce komplexního modelu zátěže (stejná jako v S1) ---
function controller_workload = calculate_complete_workload_v4(active_aircraft_positions, comm_events, current_minute, conflicts_detected, comm_backlog, base_workload, interval_duration_s)
    ...
    % 4. Konfliktní zátěž na základě vzdáleností
...
% --- Kontrola zátěže s komplexním modelem (stejná jako v S1) ---
