...
% Verze 3.0: S prioritními odletovými sloty
% Založeno na verzi 2.0 s přidáním prioritních časů pro odlety
...
THEORETICAL_HOURLY_CAPACITY = 40; % Upravte podle potřeby
...
% Povolení k přistání - TOTO ODSTRAŇUJEME (není v Čáslavi)
% comm_load_params.landing_clearance - nebude použito
...
disp(' ');
disp('--- Generování a plánování letů s prioritními sloty ---');
if ENABLE_PRIORITY_DEPARTURE_SLOTS
    disp(['Konfigurace: ', num2str(FIXED_NUM_AIRCRAFT), ' příletů + ', num2str(FIXED_NUM_DEPARTURES), ' odletů (z toho ', num2str(length(PRIORITY_DEPARTURE_TIMES_S)), ' prioritních)']);
else
    disp(['Konfigurace: ', num2str(FIXED_NUM_AIRCRAFT), ' příletů + ', num2str(FIXED_NUM_DEPARTURES), ' odletů']);
end
...
% Volání plánovacího algoritmu s prioritními sloty nebo bez nich
if ENABLE_PRIORITY_DEPARTURE_SLOTS
    [scheduled_flights, rejected_flights_info, final_workload_per_interval, runway_operations_log] = ...
        plan_flights_with_priority_slots_kbely(orig_arrival_times, orig_departure_times, ...
        ...
        td_RWY_VEC_s, PRIORITY_DEPARTURE_TIMES_S, PRIORITY_PROTECTION_BEFORE_S, PRIORITY_PROTECTION_AFTER_S);
else
    [scheduled_flights, rejected_flights_info, final_workload_per_interval, runway_operations_log] = ...
...
disp(['Úspěšně naplánováno: ', num2str(length(scheduled_arrivals)), ' příletů a ', num2str(length(scheduled_departures)), ' odletů']);
...
disp(' ');
disp('--- Dynamická simulace a vizualizace ---');

if DRAW_ANIMATION
    figure('Name','Dynamická simulace letů v TMA Kbely - S prioritními sloty','Position',[100,100,1200,800]); hold on;
    ...
    title_str = ['TMA Kbely - Separace ',num2str(MIN_SEPARATION_NM_AIR),' NM'];
    if ENABLE_PRIORITY_DEPARTURE_SLOTS
    ...
    if ~isempty(scheduled_departures)
        for i=1:length(scheduled_departures)
            ...
            if isfield(scheduled_departures(i), 'is_priority') && scheduled_departures(i).is_priority
                departure_markers(i)=plot(NaN,NaN,'rs','MarkerSize',10,'MarkerFaceColor','r','MarkerEdgeColor','r');
                departure_labels(i)=text(NaN,NaN,'','FontSize',9,'Color','r','FontWeight','bold');
            else
    ...
    disp('Spouštím animaci průběhu letů...');
...
disp(' ');
disp('========== ROZŠÍŘENÉ STATISTICKÉ VÝSLEDKY ==========');

% Základní statistiky
disp(' ');
disp('--- ZÁKLADNÍ STATISTIKY ---');
disp(['Celkem požadavků: ', num2str(FIXED_NUM_AIRCRAFT + FIXED_NUM_DEPARTURES)]);
disp(['Úspěšně naplánováno: ', num2str(length(scheduled_flights)), ' letů']);
disp(['Odmítnuto: ', num2str(length(rejected_flights_info)), ' letů']);
disp(['Dokončeno: ', num2str(landing_count), ' přistání, ', num2str(takeoff_count), ' vzletů']);
...
% Kapacitní statistiky
total_movements = landing_count + takeoff_count;
effective_hourly_capacity = total_movements / (SIM_DURATION_S / 3600);
capacity_utilization_percent = (effective_hourly_capacity / THEORETICAL_HOURLY_CAPACITY) * 100;

disp(' ');
disp('--- KAPACITNÍ ANALÝZA ---');
disp(['Efektivní hodinová kapacita: ', num2str(effective_hourly_capacity, '%.2f'), ' pohybů/hod']);
disp(['Teoretická kapacita: ', num2str(THEORETICAL_HOURLY_CAPACITY), ' pohybů/hod']);
disp(['Využití kapacity: ', num2str(capacity_utilization_percent, '%.1f'), '%']);

% Použití finálního workload profilu
workload_data_for_stats = final_workload_per_interval;
if DRAW_ANIMATION && exist('current_dynamic_workload_profile', 'var')
    workload_data_for_stats = current_dynamic_workload_profile;
end
...
disp(' ');
disp('--- KOMUNIKAČNÍ STATISTIKY ---');
disp(['Celkový čas komunikace: ', num2str(total_comm_time_s, '%.1f'), ' s (', num2str(total_comm_time_s/60, '%.2f'), ' min)']);
disp(['Vytížení frekvence: ', num2str(frequency_occupancy_percent, '%.1f'), '%']);
disp(['Průměrná doba letounu na frekvenci: ', num2str(avg_time_on_frequency_s, '%.1f'), ' s (', num2str(avg_time_on_frequency_s/60, '%.2f'), ' min)']);
...
rejection_rate_percent = (length(rejected_flights_info) / (FIXED_NUM_AIRCRAFT + FIXED_NUM_DEPARTURES)) * 100;

disp(' ');
disp('--- EFEKTIVITA PLÁNOVÁNÍ ---');
disp(['Míra odmítnutí: ', num2str(rejection_rate_percent, '%.1f'), '%']);
disp(['Průměrné zpoždění: ', num2str(avg_delay_s, '%.1f'), ' s (', num2str(avg_delay_s/60, '%.2f'), ' min)']);
disp(['Maximální zpoždění: ', num2str(max_delay_s, '%.1f'), ' s (', num2str(max_delay_s/60, '%.2f'), ' min)']);
disp(['Směrodatná odchylka zpoždění: ', num2str(std_delay_s, '%.1f'), ' s']);

% Analýza workloadu
workload_mean = mean(workload_data_for_stats);
workload_max = max(workload_data_for_stats);
workload_min = min(workload_data_for_stats);
workload_std = std(workload_data_for_stats);

time_low_percent = sum(workload_data_for_stats < LOW_WORKLOAD_THRESHOLD) / length(workload_data_for_stats) * 100;
time_medium_percent = sum(workload_data_for_stats >= MEDIUM_WORKLOAD_THRESHOLD & workload_data_for_stats < HIGH_WORKLOAD_THRESHOLD) / length(workload_data_for_stats) * 100;
time_high_percent = sum(workload_data_for_stats >= HIGH_WORKLOAD_THRESHOLD & workload_data_for_stats < OVERLOAD_THRESHOLD) / length(workload_data_for_stats) * 100;
time_overload_percent = sum(workload_data_for_stats >= OVERLOAD_THRESHOLD) / length(workload_data_for_stats) * 100;

disp(' ');
disp('--- ANALÝZA ZÁTĚŽE ---');
disp(['Průměrná zátěž: ', num2str(workload_mean, '%.1f'), ' bodů']);
disp(['Maximální zátěž: ', num2str(workload_max, '%.1f'), ' bodů']);
disp(['Minimální zátěž: ', num2str(workload_min, '%.1f'), ' bodů']);
disp(['Směrodatná odchylka: ', num2str(workload_std, '%.1f'), ' bodů']);
disp(['Čas v nízkém zatížení (<60): ', num2str(time_low_percent, '%.1f'), '%']);
disp(['Čas ve středním zatížení (90-162): ', num2str(time_medium_percent, '%.1f'), '%']);
disp(['Čas ve vysokém zatížení (162-210): ', num2str(time_high_percent, '%.1f'), '%']);
disp(['Čas v přetížení (>210): ', num2str(time_overload_percent, '%.1f'), '%']);
...
if ~isempty(workload_data_for_stats) && ~isempty(active_aircraft_count_per_interval)
    min_length = min(length(workload_data_for_stats), length(active_aircraft_count_per_interval));
    workload_trimmed = workload_data_for_stats(1:min_length);
...
disp(' ');
disp('--- KORELACE ZÁTĚŽE ---');
if ~isnan(correlation_value)
    disp(['Korelace workload vs. počet letounů: ', num2str(correlation_value, '%.3f')]);
    disp(['Nárůst workloadu na jedno letadlo: ', num2str(regression_slope, '%.2f'), ' bodů']);
    disp(['Základní workload (0 letadel): ', num2str(regression_intercept, '%.2f'), ' bodů']);
else
    disp('Korelace workload vs. počet letounů: Nedostatek dat');
end

disp(' ');
disp('=================================================');

% ----------------- 6. GRAF ZÁTĚŽE ŘÍDÍCÍHO -------------------
if ~isempty(workload_data_for_stats) && any(workload_data_for_stats)
    figure('Name', 'Analýza zátěže řídícího - TMA Kbely s prioritními sloty', 'Position', [150, 150, 1100, 600]);
    time_minutes_wl = (0:length(workload_data_for_stats)-1) * (WORKLOAD_INTERVAL_S / 60);
    plot(time_minutes_wl, workload_data_for_stats, 'b-', 'LineWidth', 2);
    hold on;
    ...
    if ENABLE_PRIORITY_DEPARTURE_SLOTS && ~isempty(PRIORITY_DEPARTURE_TIMES_S)
        for pdt = PRIORITY_DEPARTURE_TIMES_S
            ...
            text(pdt/60 + 0.5, max(workload_data_for_stats)*0.95, 'P', 'Color', 'r', 'FontWeight', 'bold', 'FontSize', 12);
        end
    end
    ...
    title_str = 'Analýza pracovní zátěže řídícího - TMA Kbely';
    if ENABLE_PRIORITY_DEPARTURE_SLOTS
        title_str = [title_str, ' (s prioritními sloty)'];
    end
    title(title_str);
    ...
    max_y_val_for_graph = max([max(workload_data_for_stats) * 1.1, OVERLOAD_THRESHOLD * 1.2, 100]);
    if isempty(max_y_val_for_graph) || isnan(max_y_val_for_graph) || max_y_val_for_graph == 0
        max_y_val_for_graph = 250;
    end
...
disp(' ');
disp('Simulace TMA Kbely s prioritními sloty úspěšně dokončena!');

% ----------------- POMOCNÉ FUNKCE -------------------

% --- NOVÁ FUNKCE PRO PLÁNOVÁNÍ S PRIORITNÍMI SLOTY PRO KBELY ---
function [planned_flights, rejected_flights_log, workload_profile, runway_ops_log] = ...
    plan_flights_with_priority_slots_kbely(orig_arrivals_t, orig_departures_t, ...
    ...
    for p_idx = 1:length(priority_departure_times_s)
        ...
        [priority_flight.vector1_time, priority_flight.ekrot_time, priority_flight.exit_time] = ...
            calculate_departure_times_kbely(priority_flight.takeoff_time, dep_route);
        ...
        temp_comm_events(end+1) = struct('time', priority_flight.takeoff_time + comm_params.takeoff_clearance.timing_offset_s, ...
            'points', comm_params.takeoff_clearance.controller_workload_points, ...
            'flight_id', priority_flight.id, 'duration', comm_params.takeoff_clearance.duration_total_s);
        temp_comm_events(end+1) = struct('time', priority_flight.vector1_time + comm_params.departure_contact.timing_offset_s, ...
            'points', comm_params.departure_contact.controller_workload_points, ...
            'flight_id', priority_flight.id, 'duration', comm_params.departure_contact.duration_total_s);
        temp_comm_events(end+1) = struct('time', priority_flight.exit_time + comm_params.exit_clearance.timing_offset_s, ...
            'points', comm_params.exit_clearance.controller_workload_points, ...
            'flight_id', priority_flight.id, 'duration', comm_params.exit_clearance.duration_total_s);
        ...
        if workload_ok
            ...
            new_op_log = struct('time', priority_flight.takeoff_time, ...
                              ...
                              'detail_time', priority_flight.vector1_time);
    ...
    % === FÁZE 2: VYTVOŘENÍ SEZNAMU VŠECH ZBÝVAJÍCÍCH POŽADAVKŮ ===
    all_remaining_requests = [];
    
    % Přidat všechny přílety
    for i = 1:length(orig_arrivals_t)
        all_remaining_requests(end+1).orig_time = orig_arrivals_t(i);
        all_remaining_requests(end).type = 'arrival';
        all_remaining_requests(end).id = ['ARR_orig_', num2str(i)];
    end
    
    % Přidat zbývající odlety (které nejsou prioritní)
    remaining_departures_start_idx = priority_slots_used + 1;
    for i = remaining_departures_start_idx:length(orig_departures_t)
        all_remaining_requests(end+1).orig_time = orig_departures_t(i);
        all_remaining_requests(end).type = 'departure';
        all_remaining_requests(end).id = ['DEP_orig_', num2str(i)];
    end
    
    % Seřadit podle původního času
    if ~isempty(all_remaining_requests)
        [~, sort_idx] = sort([all_remaining_requests.orig_time]);
        sorted_requests = all_remaining_requests(sort_idx);
    else
        sorted_requests = [];
    end
    ...
    % === FÁZE 3: PLÁNOVÁNÍ ZBÝVAJÍCÍCH LETŮ ===
    disp('Fáze 2: Plánování zbývajících letů s respektováním prioritních slotů...');
    ...
    for req_idx = 1:length(sorted_requests)
        ...
        for delay_s = 0:TIME_STEP_PLANNING_S:MAX_PLANNING_DELAY_S
            ...
            if strcmp(request.type, 'arrival')
                ...
                current_flight_candidate.faf_time = calculate_faf_arrival_time_kbely(current_flight_candidate.route_index, current_flight_candidate.entry_time, arr_routes);
                ...
                current_flight_candidate.takeoff_time = NaN; current_flight_candidate.vector1_time = NaN;
                current_flight_candidate.ekrot_time = NaN; current_flight_candidate.exit_time = NaN;
                ...
                % Kontrola zakázaných zón pro přílety
                in_forbidden_zone = false;
                for z = 1:size(forbidden_zones, 1)
                    if current_flight_candidate.landing_time >= forbidden_zones(z,1) && ...
                       current_flight_candidate.landing_time <= forbidden_zones(z,2)
                        in_forbidden_zone = true;
                        break;
                    end
                end
                if in_forbidden_zone, continue; end

                % Přidání komunikačních událostí (pouze 2 pro přílety jako v Čáslavi)
                ...
                % Plánování běžného odletu
                current_flight_candidate.takeoff_time = request.orig_time + delay_s;
                
                % Kontrola zakázaných zón pro odlety
                in_forbidden_zone = false;
                for z = 1:size(forbidden_zones, 1)
                    if current_flight_candidate.takeoff_time >= forbidden_zones(z,1) && ...
                       current_flight_candidate.takeoff_time <= forbidden_zones(z,2)
                        in_forbidden_zone = true;
                        break;
                    end
                end
                ...
                [current_flight_candidate.vector1_time, current_flight_candidate.ekrot_time, current_flight_candidate.exit_time] = ...
                    calculate_departure_times_kbely(current_flight_candidate.takeoff_time, dep_route);
                ...
                % Přidání komunikačních událostí
            ...
            if ~isempty(planned_flights)
                 air_sep_ok = check_all_air_separations_kbely(current_flight_candidate, planned_flights, arr_routes, dep_route, min_sep_air_nm, time_to_vector1_s);
            end
            ...
            % Přidání do logu operací na dráze
        ...
        if ~flight_planned_successfully
            ...
            if strcmp(request.type, 'departure')
                rejected_flights_log(end).reason = 'Nelze naplánovat - konflikt s prioritními sloty nebo nedostatek kapacity';
            else
...
% --- Zbytek pomocných funkcí zůstává stejný jako v původním skriptu ---
% (Všechny ostatní funkce z původního KBELY_NEW_TEST.m zůstávají beze změny)
...
% --- Integrovaný plánovací algoritmus s komplexním modelem ---
function [planned_flights, rejected_flights_log, workload_profile, runway_ops_log] = ...
    ...
    % Vytvoření jednotného seznamu všech požadavků
    all_requests = [];
    for i = 1:length(orig_arrivals_t)
    ...
    planned_flights = struct('id', {}, 'type', {}, 'route_index', {}, ...
                             'entry_time', {}, 'faf_time', {}, 'landing_time', {}, ...
                             'takeoff_time', {}, 'vector1_time', {}, 'ekrot_time', {}, 'exit_time', {}, ...
    ...
    MAX_PLANNING_DELAY_S = 600;
    TIME_STEP_PLANNING_S = 15; % Jemnější krok

    % Plánování jednotlivých letů
    for req_idx = 1:length(sorted_requests)
        request = sorted_requests(req_idx);
        flight_planned_successfully = false;
        ...
        if ~flight_planned_successfully
            ...
            rejected_flights_log(end).type = request.type;
            rejected_flights_log(end).orig_time = request.orig_time;
            rejected_flights_log(end).reason = 'Nenalezen slot splňující všechny požadavky';
        end
    end
